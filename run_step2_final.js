...
async function runStep2Final() {
  ...
  try {
    ...
    // Get first 5 KKAs and assign them to the assessment
    const kkaListResult = await client.query(`
      SELECT id, kode, nama 
      FROM kka 
    ...
    console.log(`   Found ${kkaListResult.rows.length} KKAs to assign:`);
    kkaListResult.rows.forEach(row => {
      console.log(`   - ${row.kode}: ${row.nama}`);
    });

    // Assign assessment_id to these KKAs
    for (const kka of kkaListResult.rows) {
      await client.query('UPDATE kka SET assessment_id = $1 WHERE id = $2', [assessmentId, kka.id]);
    }
    console.log(`✅ Assigned assessment_id to ${kkaListResult.rows.length} KKAs`);
    ...
    if (specificAssessment.rows.length > 0) {
      const row = specificAssessment.rows[0];
...
runStep2Final().catch(console.error);
